...
def generate_launch_description():
    ...
    recognition_node = Node(
        ...
        parameters=[{
            ...
            # The node selects the correct model based on inference_mode.
            # Pass both; the launch file picks the right one via a Python expression
            # below. For simplicity we pass the cpu model as default and let the
            # user override model_path if needed, OR we use a script-level trick.
            # Simplest: pass both paths as separate params and let the Python node
            # pick the right one.
